...
nitobi.ui.Toolbars.prototype.initialize= function() 
{
	...
	this.toolbarXml = nitobi.xml.createXmlDoc(nitobi.xml.serialize(nitobi.treegrid.toolbarDoc));
	this.toolbarPagingXml = nitobi.xml.createXmlDoc(nitobi.xml.serialize(nitobi.treegrid.pagingToolbarDoc));
}
...
nitobi.ui.Toolbars.prototype.findCssUrl = function()
{
	...
	if (sheet==null)
	{
		...
		if (sheet==null)
		{	
			nitobi.lang.throwError("The CSS for the toolbar could not be found.  Try moving the nitobi.treegrid.css file to a location accessible to the browser's javascript or moving it to the top of the stylesheet list. findParentStylesheet returned " + sheet);
		}
	}
