...
//		this.surface.style.height = this.grid.getHeight() + "px";
	...
	//var gridLeft = (nitobi.browser.IE?this.grid.UiContainer.getBoundingClientRect().left:0);
	this.lineStyle.left = x + "px";

 	// Fit the line in the viewable area. 26 for the scrollbar
 	var surface = column.surface;
 	var ss = this.grid.scroller.scrollSurface;
 	var scrollTop = ss.scrollTop;
 	var heightOffset = (surface.headerAttached?23:surface.calculateOffsetTop() - scrollTop);
	this.lineStyle.height = ss.offsetHeight - (surface.key == "0"?0:heightOffset) + "px";	

  	// Align the resize line to the column header element.
  	var topOffset = (surface.key == "0"?0:scrollTop);
  	nitobi.drawing.align(this.line,columnHeaderElement,nitobi.drawing.align.ALIGNTOP,0,0,nitobi.html.getHeight(columnHeaderElement) + 1 - (nitobi.browser.MOZ && !surface.headerAttached?topOffset:0));
