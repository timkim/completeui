...
<xsl:variable name="offset">
    ...
    &lt;div id="grid<xsl:value-of select="$u" />" class="ntb-grid ntb-grid-reset <xsl:value-of select="//ntb:treegrid/@Theme" />" style="overflow:visible;"&gt;
    ...
    &lt;table cellpadding="0" cellspacing="0" border="0" &gt;
        &lt;tr&gt;
            &lt;td id="ntb-grid-scroller<xsl:value-of select="$u" />" class="ntb-grid-scrollerheight<xsl:value-of select="$u" /> ntb-grid-scrollerwidth<xsl:value-of select="$u" />" &gt;
                &lt;div id="ntb-grid-scrollerarea<xsl:value-of select="$u" />" class="ntb-grid-scrollerheight<xsl:value-of select="$u" />" style="overflow:hidden;" &gt;
                    &lt;div tabindex="2" id="ntb-grid-surface-container-<xsl:value-of select="$u" />" class="ntb-grid-scroller<xsl:value-of select="$u" /> ntb-grid-scrollerheight<xsl:value-of select="$u" />" &gt;
        ...
        &lt;tr id="ntb-grid-hscrollshow<xsl:value-of select="$u" />" &gt;
            &lt;td &gt;&lt;div id="hscrollclip<xsl:value-of select="$u" />" class="ntb-grid-scrollbarheight<xsl:value-of select="$u" /> ntb-grid-scrollerwidth<xsl:value-of select="$u" /> ntb-hscrollbar" style="overflow:hidden;" &gt;
    ...
    &lt;/table&gt;
		...
		&lt;div id="toolbarContainer<xsl:value-of select="$u" />" style="overflow:hidden;" class="ntb-grid-toolbarshow<xsl:value-of select="$u" /> ntb-grid-toolbarheight<xsl:value-of select="$u" /> ntb-grid-width<xsl:value-of select="$u" /> ntb-toolbar"&gt;&lt;/div&gt;

        &lt;div id="ntb-grid-toolscontainer<xsl:value-of select="$u"/>" style="height:0px;"&gt;
