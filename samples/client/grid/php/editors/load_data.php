...
*/
	...
	$getHandler->DefineField("ProductLink");  // hyperlink url
	$getHandler->DefineField("LastUpdated"); // a date field
	$getHandler->DefineField("ProductIcon"); // the image for the icon on each row
	$getHandler->DefineField("BulkAction"); // for the checkbox
	$getHandler->DefineField("ProductPassword"); //for the password field
	$getHandler->DefineField("ButtonText");	// The label on each button
	...
	for ( $counter = 0; $counter < $nrows; $counter++) {
				...
				$record->add("LastUpdated",	 "2006-09-14"); // Just a random date value for demonstration purposes
